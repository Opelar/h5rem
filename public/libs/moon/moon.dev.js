...
(function(root, factory) {
   ...
   * @return {Object} Virtual DOM Node
   */
  var createFunctionalComponent = function(
    props,
    children,
    functionalComponent
  ) {
  ...
  var diffProps = function(node, nodeProps, vnode) {
    ...
    for (var vnodePropName in vnodeProps) {
      ...
      if (
        vnodePropValue !== undefined &&
        vnodePropValue !== false &&
        vnodePropValue !== null &&
        (nodePropValue === undefined ||
          nodePropValue === false ||
          nodePropValue === null ||
          vnodePropValue !== nodePropValue)
      ) {
        if (vnodePropName.length === 10 && vnodePropName === "xlink:href") {
          ...
          node.setAttribute(
            vnodePropName,
            vnodePropValue === true ? "" : vnodePropValue
          );
        }
      }
    }
    ...
    for (var nodePropName in nodeProps) {
      ...
      if (
        vnodePropValue$1 === undefined ||
        vnodePropValue$1 === false ||
        vnodePropValue$1 === null
      ) {
  ...
  var hydrate = function(node, vnode, parent) {
    ...
    if (node === null) {
      ...
      while (vchild !== null || currentChildNode !== null) {
        var next =
          currentChildNode !== null ? currentChildNode.nextSibling : null;
  ...
  var parseWalk = function(state) {
    ...
    if (token.type === "tag") {
      ...
      var isVoidElement =
        VOID_ELEMENTS.indexOf(tagType) !== -1 || closeEnd === true;
      ...
      if (isVoidElement === true) {
        ...
        if ("development" !== "production") {
          error(
            'Could not locate opening tag for the element "' + node.type + '"'
          );
        }
        return null;
        ...
        while (
          token.type !== "tag" ||
          (token.type === "tag" &&
            ((token.closeStart === undefined && token.closeEnd === undefined) ||
              token.value !== tagType))
        ) {
  ...
  var generateProps = function(node, parent, state) {
    ...
    for (propKey in props) {
      ...
      var name = prop.name;
      if (
        (specialDirective = specialDirectives[name]) !== undefined &&
    ...
    if (hasDirectives === true) {
      ...
      for (var i = 0; i < directiveProps.length; i++) {
        ...
        propsCode +=
          ...
          directiveProp.name +
          '": ' +
          (directivePropValue.length === 0 ? '""' : directivePropValue) +
          ", ";
      }
  ...
  var generateEventlisteners = function(eventListeners) {
    ...
    for (var type in eventListeners) {
      ...
      eventListenersCode =
        eventListenersCode.substring(0, eventListenersCode.length - 2) + "], ";
    }
  ...
  var generateNode = function(node, parent, state) {
    if (typeof node === "string") {
      ...
      return (
        'instance.$slots["' +
        (slotName === undefined ? "default" : slotName.value) +
        '"]'
      );
    } else {
      ...
      if (specialDirectivesAfter !== null) {
        ...
        for (var specialDirectiveKey in specialDirectivesAfter) {
          ...
          call = specialDirectiveAfter.afterGenerate(
            specialDirectiveAfter.prop,
            call,
            node,
            state
          );
  ...
  var generate = function(tree) {
    ...
    for (var i = 0; i < dependencies.length; i++) {
      ...
      dependenciesCode +=
        "var " + dependency + ' = instance.get("' + dependency + '"); ';
    }

    var code =
      "var instance = this; " + dependenciesCode + "return " + rootCode + ";";
    ...
    beforeGenerate: function(prop, vnode, parentVNode, state) {
      ...
      for (var i = 0; i < rawModifiers.length; i++) {
        ...
        if (eventModifierCode === undefined) {
          modifiers +=
            'if(Moon.renderEventModifier(event.keyCode, "' +
            rawModifiers[i] +
            '") === false) {return null;};';
        } else {
  ...
  specialDirectives["m-model"] = {
    beforeGenerate: function(prop, vnode, parentVNode, state) {
        ...
        if (type === "checkbox" || (type === "radio" && (radio = true))) {
          ...
          if (radio === true) {
            ...
            if (valueAttr !== undefined) {
              valueAttrValue =
                '"' +
                compileTemplate(valueAttr.value, dependencies, true) +
                '"';
            } else if ((literalValueAttr = attrs["m-literal:value"])) {
              valueAttrValue =
                "" +
                compileTemplate(literalValueAttr.value, dependencies, true);
            }
      ...
      if (bracketIndex !== -1 || dotIndex !== -1) {
        ...
        keypathGetter =
          base +
          dynamicPath.replace(expressionRE, function(match, reference) {
